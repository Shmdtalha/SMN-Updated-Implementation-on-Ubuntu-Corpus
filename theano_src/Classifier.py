...
class HiddenLayer(object):
    def __init__(self, rng, input, n_in, n_out, W=None, b=None,
        ...
        #self.output = _dropout_from_layer(rng,self.output,0.3)
...
class TensorClassifier(object):
    ...
    def __init__(self, rng, n_left,n_right,dim_tensor=1,activate_func = 'tanh'):
        self.dim_tensor = dim_tensor
        self.activation = activate_func

        W_values = numpy.asarray(
                    numpy.random.randn(n_left,self.dim_tensor,n_right) * 0.05,
                    # rng.uniform(
                    #     low=-numpy.sqrt(6. / (n_left+n_right)),
    ...
    def __call__(self,left,right,batch_size, *args, **kwargs):
        ...
        tmp2 = T.batched_dot(T.tensordot(left,self.W,[1,0]),right)
        tmp2 = theano.tensor.reshape(tmp2,(batch_size,self.dim_tensor))
        if self.activation == 'tanh':
            return theano.tensor.tanh(tmp2 + tmp1)
        if self.activation == 'relu':
            return T.maximum(0.0, tmp2)
# start-snippet-2
class MLP(object):
    """Multi-Layer Perceptron Class
